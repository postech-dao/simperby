...
impl<N: GossipNetwork, S: Storage> DistributedMessageSet<N, S> {
    ...
    /// Removes the given messages identified by their hashes from the storage.
    ///
    /// This will notify the network client to consider the sen
    pub async fn remove_messages(&mut self, hashes: &[Hash256]) -> Result<(), Error> {
        let mut storage = self.storage.write().await;
        for hash in hashes {
            storage.remove_file(&hash.to_string()).await?;
        }
        Ok(())
    }
